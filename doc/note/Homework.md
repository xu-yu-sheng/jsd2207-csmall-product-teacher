...
- 删除类别
  - 开发至前端页面，即：可以通过点击页面中类别列表的删除按钮来删除类别
    - 提示：显示类别列表是DAY10作业内容
  - 业务规则：数据必须存在，如果关联了品牌则不允许删除，如果关联了属性模板则不允许删除，如果关联了SPU则不允许删除，如果这是某个父级类别的最后一个子级类别，需要将父级类别的is_parent修改为0
- 绑定品牌与类别
  - 本质是上向`pms_brand_category`表中插入数据
  - 开发至控制器层，即：通过API文档可以访问
  - 业务规则：品牌必须存在，类别必须存在，尝试绑定的关系原本不存在（例如已存在品牌1与类别1的关联数据，则不允许再次添加品牌1与类别1的关联数据）
- 解绑品牌与类别
  - 本质上是删除`pms_brand_category`表中的数据
  - 开发至控制器层，即：通过API文档可以访问
    - 参数应该设计为：`Long brandId, Long categoryId`，即：根据品牌id和类别id执行解绑
  - 业务规则：绑定关系必须存在（尝试删除的数据必须存在）
- 添加图片
  - 本质是上向`pms_picture`表中插入数据
    - 提示：图片上传与添加图片数据是不同的功能，此处不需要考虑图片如何上传的问题
  - 开发至控制器层，即：通过API文档可以访问
  - 业务规则：相册必须存在（图片表中有`album_id`字段）
- 删除图片
  - 本质上是删除`pms_picture`表中的数据
  - 开发至控制器层，即：通过API文档可以访问
  - 业务规则：数据必须存在
- 根据属性模板查询属性列表
  - 开发至控制器层，即：通过API文档可以访问
...
在`csmall-passport`项目中：

- 将`ams_role`表中的角色列表缓存到Redis中
  - 当客户端请求查看角色列表时，响应Redis中的结果
  - 实现重建角色数据缓存的功能（仅需通过API文档能访问即可）
...
- 显示“属性”列表
  - 显示逻辑为：根据“属性模板”来显示“属性”列表
  - 在前端界面中，先在界面上添加下拉菜单，以显示“属性模板”列表，并为此下拉菜单绑定事件，当选择了菜单项中的某项时，显示选中的“属性模板”对应的“属性”列表
- 删除属性
  - 业务规则：数据必须存在
- 修改品牌详情

以上功能完成至“可以通过前端页面查看或操作”。
